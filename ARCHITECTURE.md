...
- `agent` — `platform::agent`: агентский shell Конфигуратора по SSH встроенным
  ...
  аутентификация. Одна сессия на команду (`use_cases::agent_session`): `dump` и
  `build` ходят через неё, каталоги проекта выставляются агенту ссылкой в его
  `AgentBaseDir`, после удачной загрузки или выгрузки записывается поколение
  конфигурации, и неизменившееся поколение не выгружается. В матрице стоит
  экспериментально у `dump` и `build`.
