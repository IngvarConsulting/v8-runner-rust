...
- `agent` — `platform::agent`: агентский shell Конфигуратора через системный `ssh`
  без псевдотерминала, ответы — JSON-массивы с закрытым `error-type`. Точку входа
  ...
  аутентификация. Пока стоит в матрице экспериментально у `dump`.
