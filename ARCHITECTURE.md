...
7. `output` contains CLI presentation primitives such as `Presenter`; the response `Envelope` lives in `command_envelope`.
...
- `agent` — `platform::agent`: агентский shell Конфигуратора по SSH встроенным
  ...
  (`exchange: sftp` — подсистема того же SSH-соединения, или `exchange.dir` — тот
  каталог, видимый раннеру), `workPath` остаётся локальным. Слой обмена
  (`agent_session::Exchange`) один для всех адаптеров: выставить, забрать, убрать след.
  Там агент — единственный исполнитель.
...
- `infobase.cluster` держит адрес сервера администрирования и два уровня администраторов
  над пользователем базы (кластер, центральный сервер); у файловой базы и у автономного
  сервера секция отвергается. Её читатели — `sessions`, `ras` раннера и `infobase create`
  в кластере — приходят этапами 4–5 плана 1.0.0 (#233); до них секцию проверяет только
  валидация.
