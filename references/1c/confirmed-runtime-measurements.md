...
в [`spec/arch/rules/`](../../spec/arch/README.md).

Везде, где не сказано иначе, платформа — `8.3.27.2074`.

## Загрузка информационной базы из DT

Замер на файловой базе (выброшенные копии). Обратной операции нет ни в одном режиме:
`load` принимает только `.cf`/`.cfe`, `init` входа DT не принимает.
...
Оставшийся после отказа `1Cv8.cgr.cfl` — признак того, что каталог уже тронут.

Задача замера: [#86](https://github.com/IngvarConsulting/v8-runner-rust/issues/86).
...
В прежней записи значилось, что `generation-id` есть только у агентского shell'а. Это
неверно: у пакетного Конфигуратора есть `/GetConfigGenerationID`. Идентификатор поколения
доступен обоим исполнителям.
