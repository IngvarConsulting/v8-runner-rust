...
Ключ глобальный — его место в строке не важно, — а команда без превью (`version`, `clone`,
`init`, `tools download`, `test`, `check`, `mcp serve`) отвергает его с названной причиной.
