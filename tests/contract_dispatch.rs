...
//! утилитой платформы; `check edt` требует проекта в формате EDT.
//!
...
fn rows(dir: &Path) -> Vec<Dispatching> {
    ...
    vec![
        ...
        // Заглушка молчит, и запись о расширении не находится уже после того, как `ibcmd`
        // запрос получил: отказ после работы отвечает формой чтения.
        given(
