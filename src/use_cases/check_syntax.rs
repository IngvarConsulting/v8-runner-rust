...
fn run_syntax_with_context(
    ...
    // запись этой команды, а превью не создаёт ничего. Боевой порядок при этом остаётся
    // прежним, поэтому первым отказом у нечитаемого рабочего каталога по-прежнему
    // приходит отказ журнала, а не отказ поиска платформы.
    if args.dry_run {
...
/// до первой записи. Каталог журналов платформы не создаётся, поэтому путь журнала
/// превью не называет — файла не будет.
