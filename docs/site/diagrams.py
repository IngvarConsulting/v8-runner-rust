...
def general_map(id):
    ...
    d.edge('R', 'GATE', 'SSH · SFTP', dashed=True, path='M170,318 C300,318 480,300 600,330 C720,360 800,420 898,420', lx=640, ly=344)
    d.edge('EDT', 'SRC', fb='l:0.4')
    d.edge('DES', 'SRC', fa='r:0.3', fb='l:0.8')
    d.edge('DES', 'FDB', 'файл', fa='r:0.62', fb='l:0.3', t=0.5)
    d.edge('DES', 'CLU', 'TCP 1541', dashed=True, path='M484,150 C600,100 760,100 938,166', lx=700, ly=110)
    d.edge('DES', 'GATE', 'TCP: прямой шлюз', dashed=True, path='M484,176 C600,240 760,330 898,410', lx=640, ly=296)
    d.edge('AGT', 'FDB', 'файл', fa='r:0.3', fb='l:0.7', t=0.5)
    d.edge('AGT', 'CLU', 'TCP 1541', dashed=True, path='M520,270 C680,262 800,210 938,198', lx=700, ly=252)
    d.edge('IBC', 'FDB', '--db-path', path='M468,352 C560,352 600,215 620,206', lx=540, ly=340)
    d.edge('IBC', 'DBMS', '--dbms: прямо в СУБД', dashed=True, path='M468,368 C700,368 1150,380 1250,206', lx=760, ly=376)
    d.edge('RAC', 'RAS', 'TCP 1545: сеансы своей базы', dashed=True, path='M468,452 C640,452 780,300 968,300', lx=600, ly=440)
...
def processes(id):
    ...
    d.node('R', 'раннер\nCLI или MCP-сервер', 'run', 110, 200, w=150)
    d.node('D', '1cv8 DESIGNER', 'sw', 470, 40)
    d.node('I', 'ibcmd', 'sw', 470, 105)
    d.node('A', 'агент: 1cv8 /AgentMode\nили шлюз ibsrv', 'svc', 470, 180)
    d.node('E', '1cedtcli', 'sw', 470, 260)
    d.node('C', 'клиент 1С', 'sw', 470, 325)
    d.node('W', 'webinst', 'sw', 470, 390)
    d.edge('R', 'D', 'процесс на операцию', fa='r:0.1', t=0.5)
    d.edge('R', 'I', 'процесс на операцию', fa='r:0.25', t=0.5)
    d.edge('R', 'A', 'сессия SSH', fa='r:0.42', t=0.5)
    d.edge('R', 'E', 'одна живая сессия', fa='r:0.58', t=0.5)
    d.edge('R', 'C', 'процесс на запуск', fa='r:0.75', t=0.5)
    d.edge('R', 'W', 'процесс на операцию', fa='r:0.9', t=0.5)
    return d
processes('processes')
