...
def processes(id):
    ...
    d.node('D', '1cv8 DESIGNER', 'sw', 450, 60)
    d.node('I', 'ibcmd', 'sw', 680, 110)
    d.node('A', 'агент: 1cv8 /AgentMode\nили шлюз ibsrv', 'svc', 790, 270)
    d.node('E', '1cedtcli', 'sw', 680, 430)
    d.node('C', 'клиент 1С', 'sw', 450, 480)
    d.node('W', 'webinst', 'sw', 220, 430)
    d.node('RC', 'rac', 'sw', 110, 270)
    d.node('RS', 'ras cluster\nесли cluster.ras не объявлен', 'svc', 220, 110)
    d.edge('R', 'D', 'процесс на операцию', fa='t', fb='b', t=0.4)
    d.edge('R', 'I', 'процесс на операцию', fa='r:0.15', fb='l:0.7', t=0.72)
    d.edge('R', 'A', 'сессия SSH', fa='r:0.5', fb='l', t=0.55)
    d.edge('R', 'E', 'одна живая сессия', fa='r:0.85', fb='l:0.3', t=0.72)
    d.edge('R', 'C', 'процесс на запуск', fa='b', fb='t', t=0.4)
    d.edge('R', 'W', 'процесс на операцию', fa='l:0.85', fb='r:0.3', t=0.78)
    d.edge('R', 'RC', 'процесс на операцию', fa='l:0.5', fb='r', t=0.55)
    d.edge('R', 'RS', 'поднимает на время команды', fa='l:0.15', fb='r:0.7', t=0.78)
    return d
processes('processes')
