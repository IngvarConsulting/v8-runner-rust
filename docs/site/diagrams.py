...
def general_map(id):
    ...
    d.node('FDB', 'файловая база\nкаталог с 1Cv8.1CD', 'doc', 740, 208, w=200)
    d.node('GATE', 'автономный сервер ibsrv\nпрямой шлюз · SSH-шлюз 1543', 'svc', 1040, 60, w=280)
    d.node('CLU', 'кластер 1С\nragent · rmngr · rphost', 'sw', 1040, 208, w=200)
    d.node('DBMS', 'СУБД', 'cyl', 1040, 330, w=90, h=54)
    d.node('RAS', 'RAS 1545', 'svc', 1250, 208)
    ...
    d.edge('R', 'RAS', 'поднимает, если cluster.ras не объявлен', path='M100,329 L100,600 Q100,630 130,630 L1350,630 Q1380,630 1380,600 L1380,238 Q1380,208 1350,208 L1304,208', lx=1100, ly=630)
    d.edge('EDT', 'SRC', fb='l:0.4')
    d.edge('V8', 'SRC', fa='r:0.1', fb='l:0.75')
    d.edge('V8', 'FDB', 'файл', fa='r:0.6', fb='l:0.5', t=0.5)
    d.edge('V8', 'CLU', 'TCP 1541', dashed=True, path='M530,167 C640,140 800,140 940,196', lx=735)
    d.edge('V8', 'GATE', 'TCP: прямой шлюз', dashed=True, path='M530,158 C620,108 760,108 830,108 C880,108 885,60 900,60', lx=740)
    d.edge('IBC', 'FDB', '--db-path', fa='r:0.3', fb='l:0.85', lx=594)
    d.edge('IBC', 'DBMS', '--dbms: прямо в СУБД', dashed=True, fa='r:0.7', fb='l', lx=700)
    d.edge('RAC', 'RAS', 'TCP 1545: сеансы своей базы', dashed=True, path='M488,400 C800,400 1250,430 1250,230', lx=700)
...
def d_file(id):
    ...
    d.node('IB', 'каталог базы\n1Cv8.1CD', 'doc', 720, 200, w=170)
    d.node('WP', 'workPath\nжурналы, хеши, .cf, .dt', 'doc', 720, 60, w=210)
    ...
    d.edge('DES', 'IB', 'файл, монопольно', fa='r:0.75', fb='l:0.2', t=0.5)
    d.edge('IBC', 'IB', '--db-path', fb='l:0.45')
    d.edge('AGT', 'IB', 'файл', fa='r:0.3', fb='l:0.75', t=0.5)
    d.edge('CLI', 'IB', 'файл', fb='b:0.3', t=0.5)
    d.edge('DES', 'WP', fa='r:0.35', fb='l:0.5')
    return d
d_file('d-file')
...
def d_file_web(id):
    ...
    d.node('PUB', 'каталог публикации\ndefault.vrd', 'doc', 720, 220, w=190)
    d.node('IB', 'каталог базы\n1Cv8.1CD', 'doc', 720, 360, w=170)
    ...
    d.edge('WEBI', 'PUB', '-dir, -wsdir, -connstr File=…', path='M430,92 C520,92 560,208 625,208', lx=560)
    d.edge('WEBI', 'WS', '-confpath: правка конфига', fa='r:0.5', fb='l:0.5', t=0.5)
    d.edge('WS', 'IB', 'файл', path='M837,109 C837,220 880,354 805,354', t=0.5)
    d.edge('WS', 'PUB', fa='b:0.42', fb='t')
...
def d_cluster_local(id):
    ...
    d.node('AGT', '1cv8 /AgentMode\n127.0.0.1:1543', 'svc', 380, 175)
    d.node('CLI', '1cv8c · клиент', 'sw', 380, 275)
    d.node('IBC', 'ibcmd', 'sw', 380, 360)
    d.node('WP', 'workPath\n.cf, .dt, журналы', 'doc', 620, 80, w=170)
    d.frame('HOST', 'раннер и утилиты', ['R', 'DES', 'AGT', 'CLI', 'IBC', 'WP'])
    d.node('RAG', 'ragent 1540\nrmngr 1541', 'sw', 820, 170)
    d.node('RPH', 'rphost 1560–1591', 'sw', 820, 290)
    d.frame('SRV', 'сервер 1С', ['RAG', 'RPH'], kind='srv')
    d.frame('BOX', 'одна машина', ['R', 'DES', 'AGT', 'CLI', 'IBC', 'WP', 'RAG', 'RPH'], kind='box', pad=44, top=52)
    d.node('DBMS', 'СУБД', 'cyl', 1140, 290, w=90, h=54)
    host_fan(d, [('DES', FAN, 'r:0.15', 0.7), ('AGT', 'поднимает, потом SSH', 'r:0.4', 0.5), ('CLI', 'процесс на запуск', 'r:0.65', 0.7), ('IBC', FAN, 'r:0.9', 0.7)])
    d.edge('DES', 'WP', fa='r:0.3', fb='l')
    d.edge('DES', 'RAG', 'Srvr=…;Ref=…, TCP', path='M445,90 C560,140 690,150 764,160', lx=600)
    d.edge('AGT', 'RAG', 'TCP', fa='r:0.4', fb='l:0.5', t=0.6)
    d.edge('CLI', 'RAG', 'TCP', fb='l:0.85', t=0.6)
    d.edge('IBC', 'DBMS', '--dbms, --database-server', path='M428,360 C600,360 700,400 840,400 C980,400 1080,380 1140,318', lx=840, ly=400)
