...
class Diagram:
    ...
    def edge_geometry(self, e):
        if e['lx'] is not None and e['path']:
        ...
        if e['lx'] is not None: return path, (e['lx'], e['ly'])
...
def general_map(id):
    ...
    d.node('EDT', '1cedtcli', 'sw', 420, 60)
    d.node('DES', '1cv8 DESIGNER', 'sw', 420, 160)
    d.node('AGT', '1cv8 /AgentMode\n127.0.0.1:1543', 'svc', 420, 262)
    d.node('IBC', 'ibcmd', 'sw', 420, 362)
    d.node('RAC', 'rac', 'sw', 420, 452)
    d.node('WEB', 'webinst', 'sw', 420, 542)
    d.frame('HOST', 'машина раннера', ['R', 'EDT', 'DES', 'AGT', 'IBC', 'RAC', 'WEB'])
    ...
    d.node('DBMS', 'СУБД', 'cyl', 1250, 178, w=90, h=54)
    d.node('RAS', 'RAS 1545', 'svc', 1040, 300)
    d.node('GATE', 'автономный сервер ibsrv\nпрямой шлюз · SSH-шлюз 1543', 'svc', 1040, 420, w=280)
    d.node('WS', 'Apache · IIS', 'svc', 740, 542)
    d.edge('R', 'EDT', 'одна живая сессия', fa='r:0.1', t=0.72)
    d.edge('R', 'DES', 'процесс на операцию', fa='r:0.3', t=0.72)
    d.edge('R', 'AGT', 'поднимает, потом SSH', fa='r:0.5', t=0.5)
    d.edge('R', 'IBC', 'процесс на операцию', fa='r:0.7', t=0.72)
    d.edge('R', 'RAC', 'процесс на операцию', fa='r:0.85', t=0.72)
    d.edge('R', 'WEB', 'процесс на операцию', fa='r:0.95', t=0.74)
    d.edge('R', 'RAS', 'поднимает, если cluster.ras не объявлен', path='M100,330 L100,610 Q100,640 130,640 L1185,640 Q1215,640 1215,610 L1215,330 Q1215,300 1185,300 L1112,300', lx=640, ly=640)
    d.edge('R', 'GATE', 'SSH · SFTP', dashed=True, path='M170,318 C300,318 480,300 600,330 C720,360 800,420 898,420', t=0.3)
    d.edge('EDT', 'SRC', fb='l:0.4')
    d.edge('DES', 'SRC', fa='r:0.3', fb='l:0.8')
    d.edge('DES', 'FDB', 'файл', fa='r:0.62', fb='l:0.3', t=0.5)
    d.edge('DES', 'CLU', 'TCP 1541', dashed=True, path='M484,150 C600,100 760,100 938,166', t=0.35)
    d.edge('DES', 'GATE', 'TCP: прямой шлюз', dashed=True, path='M484,176 C600,240 760,330 898,410', t=0.12)
    d.edge('AGT', 'FDB', 'файл', fa='r:0.3', fb='l:0.7', t=0.5)
    d.edge('AGT', 'CLU', 'TCP 1541', dashed=True, path='M520,270 C680,262 800,210 938,198', t=0.12)
    d.edge('IBC', 'FDB', '--db-path', path='M468,352 C560,352 600,215 620,206', t=0.8)
    d.edge('IBC', 'DBMS', '--dbms: прямо в СУБД', dashed=True, path='M468,368 C700,368 1150,380 1250,206', t=0.15)
    d.edge('RAC', 'RAS', 'TCP 1545: сеансы своей базы', dashed=True, path='M468,452 C640,452 780,300 968,300', t=0.2)
    d.edge('RAS', 'CLU', fa='t', fb='b')
    d.edge('CLU', 'DBMS')
    d.edge('WEB', 'WS', 'публикация')
    return d
...
def processes(id):
    ...
    d.node('R', 'раннер\nCLI или MCP-сервер', 'run', 110, 290, w=150)
    d.node('D', '1cv8 DESIGNER', 'sw', 500, 40)
    d.node('I', 'ibcmd', 'sw', 500, 105)
    d.node('A', 'агент: 1cv8 /AgentMode\nили шлюз ibsrv', 'svc', 500, 180)
    d.node('E', '1cedtcli', 'sw', 500, 260)
    d.node('C', 'клиент 1С', 'sw', 500, 325)
    d.node('W', 'webinst', 'sw', 500, 390)
    d.node('RC', 'rac', 'sw', 500, 455)
    d.node('RS', 'ras cluster\nесли cluster.ras не объявлен', 'svc', 500, 530)
    d.edge('R', 'D', 'процесс на операцию', fa='r:0.08', t=0.6)
    d.edge('R', 'I', 'процесс на операцию', fa='r:0.2', t=0.72)
    d.edge('R', 'A', 'сессия SSH', fa='r:0.32', t=0.6)
    d.edge('R', 'E', 'одна живая сессия', fa='r:0.44', t=0.72)
    d.edge('R', 'C', 'процесс на запуск', fa='r:0.56', t=0.6)
    d.edge('R', 'W', 'процесс на операцию', fa='r:0.68', t=0.72)
    d.edge('R', 'RC', 'процесс на операцию', fa='r:0.8', t=0.6)
    d.edge('R', 'RS', 'поднимает на время команды', fa='r:0.92', t=0.72)
    return d
processes('processes')
...
def states_inside(id):
    ...
    d.node('Y', 'обновлено динамически', 'sw', 800, 120, w=210)
    d.edge('S2', 'P', 'push --no-apply', path='M150,104 C230,40 310,40 350,104', t=0.5, dy=-16)
    d.edge('S2', 'P', 'правка в Конфигураторе', path='M150,112 C230,70 320,70 350,112', t=0.5, dy=-14)
    d.edge('S2', 'P', 'upload', fa='r:0.62', fb='l:0.62', t=0.5, dy=-2)
    d.edge('P', 'S2', 'apply', path='M350,136 C300,190 200,190 150,136', t=0.5, dy=18)
    d.edge('P', 'Y', 'apply, когда сеансы не закрыть', fa='r:0.4', fb='l:0.4', t=0.5, dy=-16)
    d.edge('Y', 'S2', 'перезапуск сеансов', path='M760,142 C650,230 250,230 150,142', t=0.5, dy=18)
    return d
...
def host_fan(d, targets, x=100, y=200):
    """раннер слева и веер рёбер к утилитам: targets = [(id, label, fa, t)]"""
    for tid, label, fa, t in targets:
        d.edge('R', tid, label, fa=fa, t=t)
...
def d_file_web(id):
    ...
    d.node('BR', 'браузер · тонкий клиент', 'svc', 1080, 80)
    ...
    d.edge('BR', 'WS', 'HTTP · HTTPS, ws=…', dashed=True, fa='l', fb='r', t=0.5)
    return d
...
def d_cluster_local(id):
    ...
    d.edge('IBC', 'DBMS', '--dbms, --database-server', path='M428,180 C560,180 620,430 840,430 C980,430 1080,380 1140,318', lx=720, ly=430)
...
def d_cluster_remote(id):
    ...
    d.edge('CLI', 'RAG', 'TCP', dashed=True, fb='l:0.8', t=0.65)
...
def d_ras(id):
    ...
    d.node('DES', '1cv8 DESIGNER', 'sw', 380, 80)
    d.node('RAC', 'rac', 'sw', 380, 240)
    d.frame('HOST', 'машина раннера', ['R', 'DES', 'RAC'])
    d.node('RAG', 'ragent 1540 · rmngr 1541', 'sw', 780, 80, w=220)
    d.node('RPH', 'rphost', 'sw', 780, 200)
    d.frame('SRV', 'сервер 1С', ['RAG', 'RPH'], kind='srv')
    d.node('RAS', 'RAS 1545', 'svc', 1100, 240)
    d.node('CON', 'консоль кластера', 'off', 1100, 350, w=170)
    d.frame('ADMIN', 'машина с RAS', ['RAS', 'CON'], kind='neutral')
    host_fan(d, [('DES', FAN, 'r:0.25', 0.7), ('RAC', FAN, 'r:0.75', 0.7)])
    d.edge('DES', 'RAG', 'TCP 1541, Srvr=…;Ref=…', dashed=True, fb='l:0.5', t=0.5)
    d.edge('RAC', 'RAS', 'TCP 1545, cluster.ras', dashed=True, t=0.5)
    d.edge('CON', 'RAS', fa='t', fb='b')
    d.edge('RAS', 'RAG', 'TCP 1540', dashed=True, path='M1100,218 C1100,140 980,88 890,88', lx=1000, ly=150)
    d.edge('RAG', 'RPH', fa='b', fb='t')
    return d
d_ras('d-ras')
...
def d_ras_managed(id):
    ...
    d.node('RAG', 'ragent 1540 · rmngr 1541', 'sw', 840, 120, w=220)
    d.node('RPH', 'rphost', 'sw', 840, 240)
    d.frame('SRV', 'сервер 1С', ['RAG', 'RPH'], kind='srv')
    ...
    d.edge('DES', 'RAG', 'TCP 1541, Srvr=…;Ref=…', dashed=True, fb='l:0.3', t=0.5)
    d.edge('RAS', 'RAG', 'TCP 1540, хост из Srvr=', dashed=True, path='M515,330 C660,330 700,150 730,140', lx=640, ly=290)
    d.edge('RAG', 'RPH', fa='b', fb='t')
    return d
...
def d_cluster_web(id):
    ...
    d.node('CLI', '1cv8c · клиент', 'sw', 380, 220)
    d.node('WS', 'Apache · IIS\nмодуль wsap · wsisapi', 'svc', 740, 80, w=230)
    d.node('PUB', 'каталог публикации\ndefault.vrd', 'doc', 740, 330, w=190)
    d.frame('HOST', 'машина раннера', ['R', 'WEBI', 'CLI', 'WS', 'PUB'])
    d.node('RAG', 'ragent · rmngr 1541', 'sw', 1080, 220, w=190)
    d.node('RPH', 'rphost', 'sw', 1080, 340)
    d.frame('SRV', 'сервер 1С', ['RAG', 'RPH'], kind='srv')
    d.node('BR', 'браузер · тонкий клиент', 'svc', 1080, 80)
    host_fan(d, [('WEBI', FAN, 'r:0.25', 0.7), ('CLI', 'процесс на запуск', 'r:0.75', 0.7)])
    d.edge('CLI', 'RAG', 'Srvr=…;Ref=…, TCP 1541', dashed=True, fb='l:0.5', t=0.5)
    d.edge('WEBI', 'PUB', '-dir, -wsdir, -connstr Srvr=…;Ref=…', path='M430,92 C520,92 560,320 645,326', lx=560, ly=200)
    d.edge('WEBI', 'WS', '-confpath: правка конфига', fa='r:0.4', fb='l:0.5', t=0.5)
    d.edge('WS', 'PUB', fa='b', fb='t')
    d.edge('WS', 'RAG', 'TCP 1541', dashed=True, path='M855,96 C940,96 960,190 985,200', lx=950, ly=150)
...
def d_standalone_local(id):
    ...
    d.node('DES', '1cv8 DESIGNER', 'sw', 380, 300)
    d.frame('HOST', 'раннер', ['R', 'WP', 'DES'])
    ...
    d.node('BR', 'браузер · тонкий клиент', 'svc', 1320, 80)
    d.edge('R', 'GATE', 'SSH: команда строкой, ответ JSON', fa='r:0.25', fb='l:0.5', t=0.5)
    d.edge('R', 'DES', FAN, fa='r:0.75', t=0.6)
    d.edge('R', 'UD', 'exchange: dir — ссылки и копии', path='M165,168 C420,168 600,330 928,330', lx=600, ly=288)
    d.edge('R', 'WP', 'файл', fa='b', fb='t')
    d.edge('DES', 'DGATE', 'TCP', fb='l:0.6', t=0.5)
    d.edge('DGATE', 'IBSRV')
    d.edge('GATE', 'IBSRV', path='M885,90 C950,90 990,170 1000,178')
    d.edge('IBSRV', 'UD', fa='b', fb='t')
    d.edge('IBSRV', 'DB')
    d.edge('IBSRV', 'HTTPS', fa='t', fb='b')
    d.edge('BR', 'HTTPS', 'HTTP', fa='l', fb='r', t=0.5)
    return d
...
def d_standalone_remote(id):
    ...
    d.frame('HOST', 'машина раннера', ['R', 'WP', 'SRC', 'DES'])
    d.node('GATE', 'SSH-шлюз 1543', 'svc', 760, 80)
    d.node('DGATE', 'прямой шлюз', 'svc', 760, 190)
    d.node('IBSRV', 'ibsrv', 'sw', 1020, 190)
    d.node('UD', 'каталог пользователя шлюза\nпути разрешаются здесь', 'doc', 1020, 340, w=250)
    d.node('HTTPS', 'HTTP 8314', 'svc', 1020, 80)
    d.frame('SRV', 'другая машина', ['GATE', 'DGATE', 'IBSRV', 'UD', 'HTTPS'], kind='srv')
    d.node('DB', 'база данных', 'cyl', 1290, 190, w=120, h=54)
    d.node('BR', 'браузер · тонкий клиент', 'svc', 1290, 80)
    d.edge('R', 'WP', 'файл', fa='b', fb='t')
    d.edge('R', 'SRC', 'файл', fa='r:0.85', fb='l:0.5', t=0.5)
    d.edge('R', 'GATE', 'SSH: команда строкой, ответ JSON', dashed=True, fa='r:0.2', fb='l:0.5', t=0.5)
    d.edge('R', 'DES', FAN, fa='r:0.5', t=0.45)
    d.edge('DES', 'DGATE', 'TCP', dashed=True, t=0.5)
    d.edge('DGATE', 'IBSRV')
    d.edge('SRC', 'UD', 'SFTP того же соединения', dashed=True, t=0.5)
    d.edge('GATE', 'IBSRV', path='M830,90 C900,90 950,170 960,178')
    d.edge('IBSRV', 'UD', fa='b', fb='t')
    d.edge('IBSRV', 'DB')
    d.edge('IBSRV', 'HTTPS', fa='t', fb='b')
    d.edge('BR', 'HTTPS', 'HTTP', dashed=True, fa='l', fb='r', t=0.5)
    return d
...
def d_docker_standalone(id):
    ...
    d.node('BR', 'браузер · тонкий клиент', 'svc', 1210, 60, w=200)
    d.edge('R', 'WP', fa='b', fb='t')
    d.edge('R', 'GATE', 'SSH: команда строкой, ответ JSON', dashed=True, fa='r:0.15', fb='l:0.5', lx=390, ly=92)
    d.edge('R', 'DGATE', '1cv8 DESIGNER: TCP', dashed=True, fa='r:0.4', fb='l:0.5', lx=330, ly=170)
    d.edge('R', 'UD', 'exchange: sftp', dashed=True, path='M165,178 C400,178 560,300 835,300', lx=520, ly=250)
...
def d_web_remote(id):
    ...
    d.node('BR', 'браузер · тонкий клиент', 'svc', 1300, 380, w=200)
    ...
    d.edge('BR', 'AP', 'HTTP · HTTPS, ws=…', dashed=True, path='M1200,372 C1120,330 1060,150 895,148', lx=1080, ly=330)
    return d
...
def d_web_docker(id):
    ...
    d.node('CL', 'ragent · rmngr 1541', 'sw', 1330, 80, w=190)
    d.frame('SRV', 'сервер 1С', ['CL'], kind='srv')
    d.node('BR', 'браузер · тонкий клиент', 'svc', 1330, 240, w=200)
    d.edge('R', 'WI', FAN, fa='r:0.3', t=0.6)
    d.edge('WI', 'VOL', '-dir: пишет default.vrd', fa='b', fb='t', t=0.5, dx=70)
    ...
    d.edge('AP', 'CL', 'TCP 1541', dashed=True, path='M855,66 C960,20 1130,20 1235,72', lx=1045, ly=28)
    d.edge('AP', 'HTTPP', fa='r:0.5', fb='l')
    d.edge('BR', 'HTTPP', 'HTTP · HTTPS, ws=…', dashed=True, path='M1230,230 C1160,200 1100,150 1080,102', lx=1160, ly=170)
    return d
