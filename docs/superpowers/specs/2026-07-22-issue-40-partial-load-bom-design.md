...
- разделители компонентов пути остаются нативными для ОС (`\\` в Windows, `/` в Unix);
...
Так BOM не может появиться перед каждой строкой или быть записан дважды. Фиксированные BOM и `CRLF` являются контрактом Designer и не зависят от системного перевода строк. `PathBuf` продолжает выдавать нативные разделители компонентов пути, поэтому Unix-пути не преобразуются в Windows-формат и наоборот. Ошибки преобразования безопасных путей и ошибки файловой системы продолжают возвращаться через существующий `std::io::Result` и обрабатываются текущим механизмом сохранения diagnostic list file.
...
Ожидаемые относительные пути формируются через `PathBuf` и только затем преобразуются в UTF-8-байты. Тест не должен жёстко задавать `/` или `\\`, чтобы проверять один и тот же контракт в Windows и Unix с нативными разделителями путей.
...
Сценарий запускается существующим trusted happy-path CI на Ubuntu и Windows, где доступен настоящий Designer. Эта матрица проверяет, что одинаковые BOM/CRLF принимаются платформой, а относительные пути сохраняют корректный для ОС формат. Fork и Dependabot без секретов сохраняют текущий soft-skip live-пути.
...
- Один byte-level тест корректно работает с нативными разделителями путей Windows и Unix.
