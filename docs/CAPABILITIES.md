...
- **`provider_dispatched` превью не опознаёт.** Он говорит, получил ли исполнитель
  ...
  `true` приходит и тогда, когда команда упала уже после передачи работы: исполнитель
  её получил, и повтор может застать сделанное. Отказы после работы у `publish`,
  `launch` и `extensions list|info` пока отвечают общей формой отказа, без признака
