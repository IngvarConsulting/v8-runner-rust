...
- На Unix каталог snapshot создаётся с mode `0700`, а файл копии или маркер отсутствия —
  с `0600` (с учётом более строгого `umask`), до записи содержимого.
...
- Состояние совместимости имеет три публичных значения: `supported` — вопрос задан
  ...
  платформы на значение не влияют (ADR-0029): у расширения состав читается
