...
- **`provider_dispatched` превью не опознаёт.** Он говорит, получил ли исполнитель
  ...
  прерывании до передачи работы, и когда исполнителю нечего было делать. О превью
  вызывающий знает из своего `--dry-run`; у экспортной формы его называет и `mode`
  (`preview` против `apply`), а `provider_dispatched` там появляется только в превью.
  Сегодня многие команды отвечают `true` и на отказ после выбора исполнителя, хотя
  работы он не получал
  ([#312](https://github.com/IngvarConsulting/v8-runner-rust/issues/312)), а проба
  `upload`, упавшая после запуска, — `false`
  ([#309](https://github.com/IngvarConsulting/v8-runner-rust/issues/309)).
