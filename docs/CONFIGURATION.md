...
конфига. Объявленную серверную строку из ровно двух частей раннер отдаёт платформе в её
собственной форме: `Srvr=srv:1541;Ref=demo` уходит как `/S srv:1541\demo`, а `user`/`password`
— отдельными `/N`/`/P`; рядом с `/IBConnectionString` платформа 8.3.27.1936 на Windows
реквизиты не принимала — «Пользователь ИБ не идентифицирован» (#55). Строку с
дополнительными частями (`Usr=`, `Pwd=`, иное) раннер не разбирает и отдаёт целиком через
`/IBConnectionString`; реквизиты держите в `user`/`password`, а не в строке. Вид цели отвечает на три вопроса по порядку: есть секция
