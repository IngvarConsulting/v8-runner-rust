...
базой — `ibcmd` работает с её данными сам. Конфигуратору и агенту для работы с уже
существующей серверной базой секция не требуется, и валидацией она не запрашивается.
