...
непустую строку готовым server connection. Вид цели объявляется ровно один раз: `File=` —
файловая база, `Srvr=…;Ref=…` — кластер, секция [`infobase.standalone`](#infobasestandalone) —
автономный сервер; при ней `connection` должна быть пустой.
...
infobase:
  ...
  password: secret
  standalone:
    gate: 127.0.0.1:1543        # host:port SSH-шлюза
    exchange:
      dir: /srv/ib/users-data/Admin   # каталог пользователя шлюза, как его видит раннер
  web:
    url: http://localhost:8314/demo   # адрес для launch web
```
...
Сейчас канал один — `exchange.dir`: каталог пользователя шлюза на машине раннера или его
монтирование; без него любая команда отказывает валидацией. Канал SFTP через тот же шлюз
(сервер его отдаёт) — следующий шаг. `workPath` при этом остаётся на стороне раннера и не
может лежать внутри `exchange.dir`. `tools.designer_agent` к автономному серверу не
относится и с этой секцией отвергается; `infobase.dbms` — тоже.
...
`compare-cfg`), `syntax`, `init`, `publish`, `test` и запуск клиента по строке
подключения отказывают типизированно; клиент открывается через `launch web` по
`infobase.web.url`. После `update-db-cfg` сервер 15–20 с «обновляется» и отвергает
