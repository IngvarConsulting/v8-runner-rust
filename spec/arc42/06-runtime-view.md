...
1. По HTTP — сверка заголовка `Host` и учёт сессии. По stdio клиент один, и stdout несёт
...
4. Для конфигурации и расширения: изменений нет — шаг `skipped`. Иначе: [EDT — экспорт в
   ...
   отчёты в базу не идут: их `push` только готовит к `make`.
...
2. SSH встроенным клиентом. Объявленный ключ хоста сверяется; не объявлен — ключ
   принимается, а его отпечаток называется. Первая команда переводит сессию в JSON, затем
   подключение к базе.
...
4. Файлы идут каналом: общим каталогом или SFTP шлюза. В общий каталог раннер выставляет
   исходники ссылкой, файл — жёсткой ссылкой или копией; полную выгрузку агент пишет в
   отдельный подкаталог, инкрементальную и пообъектную — сквозь ссылку прямо в цель.
   Автономной цели без объявленного канала раннер отказывает до сессии.
