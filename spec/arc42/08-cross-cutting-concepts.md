...
- `infobase.dbms` нужна для создания серверной базы и для `ibcmd` на серверной базе —
  [правило](../rules/config/dbms-is-access-to-the-dbms.md). Подключение `ibcmd` строится
  только для `ibcmd` — исполнителем или пробой —
  [правило](../rules/use-cases/an-ibcmd-connection-is-built-only-for-ibcmd.md).
...
- Берут замок адаптеры: CLI, `app` у `clone`, порт MCP и сервер MCP у живой EDT-проверки;
  вложенный шаг идёт под замком внешней команды. Без замка — превью, `version` и `init`,
  который объявляет проект (не `infobase create`).
...
- Срока у команды нет. Досрочно её заканчивает оператор: Ctrl+C или SIGTERM в командной
  строке, отмена вызова в MCP. Команде, которая работает с `workPath`, сигнал только
  отменяет, процесс раннера он не убивает.
