...
10. Цель «автономный сервер», шаг 3. Сделано: секция `infobase.standalone` (`gate`,
   `exchange.dir`) объявляет вид цели, `agent` через SSH-шлюз `ibsrv` — единственный
   ...
   (`DEC.2026-09-14.ONLY-A-STANDALONE-SERVER-ANSWERS-WITHOUT-BEING-STARTED`). Осталось:
   канал `exchange: sftp` через тот же шлюз (сервер его отдаёт — замер 15.09.2026), чтобы
   работать с сервером на другой машине без общего каталога.
...
13. `DEC.2026-09-14.THE-TARGET-MAY-LIVE-ON-ANOTHER-MACHINE` реализовано на автономном
   ...
   каталога пользователя, файлы идут объявленным каналом, ключи запуска агента к
   автономному серверу отвергаются. Открыто: канал SFTP (см. пункт 10).
