...
Dump target не может содержать `workPath`: иначе замена каталога удалит открытый snapshot вместе с pending intent и workspace lock. Единственный владелец проверки — общий validator dump target; он сравнивает canonical paths, включая aliases. Обратное вложение (Designer mirror внутри `workPath`) разрешено. Reintroduction guard проверяет отказ до запуска платформы и сохранение source/snapshot для прямого и symlink-вложения, а также допустимость обратного вложения.
