...
владельце относится к этой блокировке, того, кто её держит. Второй не ждёт: он сразу, до
запуска исполнителя, отказывает с кодом `workspace_busy` на шаге `workspace lock` и ничего
не выгружает.
