...
| --- | --- | --- |
| Q-0 | Человек или AI-агент хочет проверить локальные изменения в исходниках 1С-решения | Основной цикл `push -> check/test -> diagnose` доступен через простой CLI/MCP contract без знания низкоуровневых команд платформы 1С |
| Q-1 | Разработчик запускает `push` после небольшого изменения в одном `source-set` | Система определяет только релевантные изменения и выбирает partial-режим, когда это допустимо |
...
| Q-9 | Полный `pull` или artifact export падает до publish | Старый target остаётся неизменённым, а failure указывает staging/log diagnostics |
