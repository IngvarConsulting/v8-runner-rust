...
13. Пункты разделов выше, говорящие о `builder`, действуют до реализации ADR-0030; по её завершении `builder` снимается, а эти пункты переписываются в терминах матрицы.
...
9. Обновление свойств безопасности выбирает `source-set` через `--name`, установленное
   платформенное имя — через `--installed-name`. Неизвестный `--name` не имеет fallback.
   Только отсутствие обоих селекторов означает все настроенные extension `source-set`.
   Единственный владелец выбора — `configure_extensions::resolve_targets`; CLI проверяет
   весь набор до lock/cleanup, use case — до платформы. Точные повторы исполняются один раз.
10. Превью обновления свойств возвращает планируемые `steps` с `provider_dispatched=false`
    после поиска IBCMD, без обращения к ИБ, lock и создания `workPath`. Оно не доказывает
    наличие расширения. Селекторы и превью родительской команды несовместимы с подкомандами.
