...
10. Раннер не владеет параметрами запуска автономного сервера: как и с какими флагами поднят `ibsrv` — решение пользователя (отладка веб-клиента, HTTP-сервисов и прочее). Раннер подключается к работающему серверу и проверяет готовность каждого пути (`--pid`, шлюз) перед использованием, а не выводит её из флагов запуска.
