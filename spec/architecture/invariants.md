...
1. Публичные состояния compatibility probe: `supported`, `not_supported`,
   `absent`, `unknown`.
2. Состояние probe определяется структурно — кодом выхода и наличием файла
   отчёта, — и никогда формулировкой `/Out`. Закрытый allowlist целых чистых
   диагностик, разрешённый ранее, отменён вместе с пунктами 3 и 4 ADR-0023.
...
6. Страж `tests/tool_output_contract.rs` находит такие решения в слоях `platform`,
   `parsers`, `use_cases` в трёх формах — сравнение, ветвь `match`, именованная
   константа — и держит реестр `PROSE_DEBT`, который может только сокращаться.
