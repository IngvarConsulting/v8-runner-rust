...
10. По умолчанию раннер не передаёт `ibsrv` флаг `--enable-extended-designer-features` и не использует `ibcmd --remote`; флаг включается только для операций, которым он нужен, и тогда готовность `ibcmd --pid` проверяется, а не предполагается. Основание — замер одной среды (ADR-0030), не свойство платформы; пересматривается новым замером.
11. Пункты разделов выше, говорящие о `builder`, действуют до реализации ADR-0030; по её завершении `builder` снимается, а эти пункты переписываются в терминах матрицы.
