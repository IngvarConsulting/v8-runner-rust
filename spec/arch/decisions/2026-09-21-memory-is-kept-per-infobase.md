...
supersedes: []
superseded-by: null
establishes: [INV.USE-CASES.FOREIGN-MEMORY-IS-NOT-USED]
---
...
не используется. Отправка в `test` не сбивает память о `prod`. Под базой лежит только
состояние обмена с ней — контекст `designer-<набор>`; кеш экспорта EDT и контекст
`edt-<набор>` от базы не зависят и остаются общими.
