...
Fork pull request workflows): пока у участника нет влитого PR в этот репозиторий, GitHub
держит его прогоны в `action_required` (`gh run list --commit <sha>`), и каждый ждёт одобрения
мейнтейнера. При более строгой политике одобрения ждут прогоны любого внешнего участника.
Одобряют после чтения диффа: прогон исполняет чужой код.
