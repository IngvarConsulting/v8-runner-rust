...
- `src/platform/edt_session.rs::cancellation_during_baseline_returns_queued_cancel_without_user_command`
  утверждает о мгновении, а не об условии: после отменённой базовой линии сессия иногда
  успевает записать следующую команду. Случаи записаны в #103.
...
запускался. Пока у участника нет влитого PR в этот репозиторий, GitHub держит его прогоны в
`action_required` (`gh run list --commit <sha>`), и каждый ждёт одобрения мейнтейнера. Одобряют после чтения диффа:
